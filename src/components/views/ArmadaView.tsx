...
import { Truck, Trophy, Award, ChevronRight, Crown, Shield, Medal, Flame, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
const ArmadaView: React.FC<ArmadaViewProps> = ({
    vehicleStats, theme, isDarkMode, setSelectedVehicle, formatNumber
}) => {
    ...
    const ComparisonBadge = ({ current, previous }: { current: number, previous: number | undefined }) => {
        if (previous === undefined || previous === 0) return <span className="text-[10px] text-slate-400 font-bold ml-1">New</span>;
        const delta = current - previous;
        const percent = (delta / previous) * 100;
        const isUp = delta > 0;
        const isNeutral = delta === 0;

        if (isNeutral) return <span className="text-[10px] text-slate-400 font-bold ml-1 flex items-center gap-0.5"><Minus size={10} /> 0%</span>;

        return (
            <span className={`text-[10px] font-bold ml-1 flex items-center gap-0.5 ${isUp ? 'text-emerald-500' : 'text-red-500'}`}>
                ...
                {vehicleStats.length > 1 && (
                    <div className={`p-6 rounded-2xl border ${theme.card} relative transform hover:-translate-y-2 transition-transform duration-300 order-2 md:order-1`}>
                        ...
                        <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-emerald-600 mb-1 flex items-baseline justify-center gap-1">
                                {formatNumber(vehicleStats[1].totalNetto)}
                            </div>
                            <div className="flex items-center justify-center gap-1 mb-1">
                                <span className="text-[10px] uppercase font-bold text-emerald-500/70">Kilogram</span>
                                <ComparisonBadge current={vehicleStats[1].totalNetto} previous={vehicleStats[1].prevTotalNetto} />
                            </div>
                        </div>
                    </div>
                )}
                ...
                {vehicleStats.length > 0 && (
                    <div className={`p-8 rounded-2xl border-2 border-amber-400 ${isDarkMode ? 'bg-amber-900/10' : 'bg-amber-50'} relative transform hover:-translate-y-2 transition-transform duration-300 z-10 shadow-xl shadow-amber-500/10 order-1 md:order-2`}>
                        ...
                        <div className="text-center">
                            ...
                            <div className="text-4xl font-black text-amber-600 mb-1 flex items-baseline justify-center gap-1">
                                {formatNumber(vehicleStats[0].totalNetto)}
                            </div>
                            <div className="flex items-center justify-center gap-1 mb-1">
                                <span className="text-xs uppercase font-bold text-amber-600/70">Kilogram</span>
                                <ComparisonBadge current={vehicleStats[0].totalNetto} previous={vehicleStats[0].prevTotalNetto} />
                            </div>
                        </div>
                    </div>
                )}
                ...
                {vehicleStats.length > 2 && (
                    <div className={`p-6 rounded-2xl border ${theme.card} relative transform hover:-translate-y-2 transition-transform duration-300 order-3`}>
                        ...
                        <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-emerald-600 mb-1 flex items-baseline justify-center gap-1">
                                {formatNumber(vehicleStats[2].totalNetto)}
                            </div>
                            <div className="flex items-center justify-center gap-1 mb-1">
                                <span className="text-[10px] uppercase font-bold text-emerald-500/70">Kilogram</span>
                                <ComparisonBadge current={vehicleStats[2].totalNetto} previous={vehicleStats[2].prevTotalNetto} />
                            </div>
                        </div>
                    </div>
                )}
                            ...
                            {vehicleStats.map((v, idx) => (
                                <tr key={v.nopol} onClick={() => setSelectedVehicle(v)} className={`${theme.hover} cursor-pointer group transition-colors`}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="font-bold text-emerald-600">{formatNumber(v.totalNetto)}</div>
                                        <div className="flex justify-end pt-1">
                                            <ComparisonBadge current={v.totalNetto} previous={v.prevTotalNetto} />
                                        </div>
                                    </td>
