...
import {
    Star, AlertOctagon, TrendingUp, TrendingDown, Ribbon, Minus
} from 'lucide-react';
...
const KualitasView: React.FC<KualitasViewProps> = ({
    qualityAnalytics, theme, isDarkMode, formatNumber
}) => {
    const ComparisonBadge = ({ current, previous }: { current: number, previous: number }) => {
        ...
        if (isNeutral) return <div className="flex items-center gap-0.5 text-[10px] text-slate-500 font-bold bg-white/50 px-1 rounded"><Minus size={10} /> 0</div>;

        return (
            <div className={`flex items-center gap-0.5 text-[10px] font-bold bg-white/50 px-1 rounded ${isUp ? 'text-emerald-700' : 'text-red-700'}`}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex gap-4">
                    <div className={`flex items-center bg-emerald-100 dark:bg-emerald-900/30 px-3 py-1.5 rounded-lg gap-2 border border-emerald-200 dark:border-emerald-800`}>
                        ...
                        <ComparisonBadge current={qualityAnalytics.gradeCounts.A} previous={qualityAnalytics.prevGradeCounts?.A} />
                    </div>
                    <div className={`flex items-center bg-orange-100 dark:bg-orange-900/30 px-3 py-1.5 rounded-lg gap-2 border border-orange-200 dark:border-orange-800`}>
                        ...
                        <ComparisonBadge current={qualityAnalytics.gradeCounts.B} previous={qualityAnalytics.prevGradeCounts?.B} />
                    </div>
                    <div className={`flex items-center bg-red-100 dark:bg-red-900/30 px-3 py-1.5 rounded-lg gap-2 border border-red-200 dark:border-red-800`}>
                        ...
                        <ComparisonBadge current={qualityAnalytics.gradeCounts.C} previous={qualityAnalytics.prevGradeCounts?.C} />
                    </div>
                </div>
            </div>
