    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <KPICard title="Total Netto" value={stats.totalNetto} unit="KG" icon={<Scale />} theme={theme} color="blue" trendData={sparklineData?.netto} />
                <KPICard title="Total Janjang" value={stats.totalJanjang} unit="JJG" icon={<FileSpreadsheet />} theme={theme} color="emerald" trendData={sparklineData?.janjang} />
                <KPICard title="Rata-rata BJR" value={stats.avgBJR} unit="KG" icon={<Activity />} theme={theme} color="amber" trendData={sparklineData?.bjr} />
                <KPICard title="Truk Masuk" value={stats.totalTruk} unit="Unit" icon={<Truck />} theme={theme} color="purple" trendData={sparklineData?.truk} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className={`lg:col-span-2 p-6 rounded-2xl border ${theme.card}`}>
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-base md:text-lg font-bold flex items-center gap-2"><span className="w-1 h-6 bg-emerald-500 rounded-full"></span> Tren Penerimaan</h3>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={chartData.trend}>
                                    ...
                                    {filteredEvents.map((event, idx) => {
                                        // Match event date to chart's displayDate
                                        const chartPoint = chartData.trend.find((d: any) => d.date === event.date);
                                        if (!chartPoint) return null;

                                        const eventConfig = EVENT_TYPES.find(e => e.type === event.type);
                                        return (
                                            <ReferenceLine
                                                key={idx}
                                                x={chartPoint.displayDate}
                                                stroke={eventConfig?.color}
                                                strokeDasharray="3 3"
                                                strokeOpacity={0.6}
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={chartData.peakHours} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                                    ...
                                    <Bar dataKey="count" fill="#8b5cf6" radius={[4, 4, 0, 0]}>
                                        {chartData.peakHours.map((peakEntry: any, index: number) => (
                                            <Cell key={`cell-${index}`} fill={`rgba(139, 92, 246, ${0.4 + (peakEntry.count / 10)})`} />
                                        ))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                                        {chartData.locations.map((_entry: any, index: number) => (
